...
Wildfly -> `http://192.168.99.100:9090`
...
## Generate books
Go tho the `http://192.168.99.100:9090/MVC-MVC/`and choose *configuration* link, user is redirected to *Login* page automatically. You must be logged in in order to access this page. Once user is logged in (admin/admin), he has to go to the generateBooks page and put the number of books to be generated. If the number is too big (firstly it takes a lot of time), pages returns an error (can be emprouved). If the login and/or password are not correct, there is no an error message but while accessing `Configuration` page user will asked again to insert login/password. This part may be improuved as well: if there's a mistake, an error message will pop up and user is staying in the same page.
...
If those fields are not valid, user is asked to repeat the opperation with good values. Previous values are still apperead in fields (it's filed with old values by default).

## New book
New book must has a title and author (otherwise it's not a book), summary field is not mandatory. Same restriction that for update book for number of pages and release year. If there is a mistake, user is redirected to the same page , and there is a error message appears in red/orange box.
...
4) LogoutServlet is working but it's not linked to the rest of the project
5) Red/orange box appears anyway, even if there is no error message. It looks like *if* verificaition in .jsp pages is not working. 
